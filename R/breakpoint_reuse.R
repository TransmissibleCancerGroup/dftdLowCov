...
make_segmatch <- function(coordinates, ignore_direction = TRUE) {
    ...
    expected_columns <- c("Order", "chr", "start", "end", "Type", "New.CNV_ID_ext", "Category")
    stopifnot(
        intersect(expected_columns, colnames(coordinates)) == expected_columns
    )
    ...
    shared_starts <-
        unique(coordinates[, .(commonCnvIds = paste0(sort(unique(New.CNV_ID_ext)), collapse=","),
    ...
    start_matches_end <- start_matches_end[, .(commonCnvIds = paste0(sort(unique(c(New.CNV_ID_ext, i.New.CNV_ID_ext))), collapse=","),
                                               tumourTypes = paste0(sort(unique(c(Category, i.Category))), collapse=","),
                                               nTumourTypes = uniqueN(c(Category, i.Category)),
                                               dft1Count = unique(rbindlist(list(data.table(New.CNV_ID_ext, Category), data.table(i.New.CNV_ID_ext, i.Category)), use.names=FALSE), by = "New.CNV_ID_ext")[, sum(Category=="DFT1")],
                                               dft2Count = unique(rbindlist(list(data.table(New.CNV_ID_ext, Category), data.table(i.New.CNV_ID_ext, i.Category)), use.names=FALSE), by = "New.CNV_ID_ext")[, sum(Category=="DFT2")],
                                               nonDftdCount = unique(rbindlist(list(data.table(New.CNV_ID_ext, Category), data.table(i.New.CNV_ID_ext, i.Category)), use.names=FALSE), by = "New.CNV_ID_ext")[, sum(startsWith(Category, "NonDFTD"))],
